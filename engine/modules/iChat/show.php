...
if (!$is_logged)
    $chat_cfg['refresh'] = $chat_cfg['guest_refresh'];
...
if ($user_group[$member_id['user_group']]['allow_url'])
    $tpl->copy_template = preg_replace("'\[allow_url\](.*?)\[/allow_url\]'si", "\\1", $tpl->copy_template);
else
    $tpl->copy_template = preg_replace("'\[allow_url\](.*?)\[/allow_url\]'si", "", $tpl->copy_template);

if (!$is_logged AND $chat_cfg['allow_guest'] == 'no')
    $tpl->copy_template = preg_replace("'\[no_access\](.*?)\[/no_access\]'si", "\\1", $tpl->copy_template);
else
    $tpl->copy_template = preg_replace("'\[no_access\](.*?)\[/no_access\]'si", "", $tpl->copy_template);

if ($is_logged OR $chat_cfg['allow_guest'] == 'yes')
    $tpl->copy_template = preg_replace("'\[editor_form\](.*?)\[/editor_form\]'si", "\\1", $tpl->copy_template);
else
    $tpl->copy_template = preg_replace("'\[editor_form\](.*?)\[/editor_form\]'si", "", $tpl->copy_template);
