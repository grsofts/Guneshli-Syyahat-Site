...
/*
====================================================
јвтор: RooTM http://weboss.net/
----------------------------------------------------
јдаптаци¤ под DLE: 10.4 SX2 http://sx2.name/
====================================================
*/
...
//################# ќпределение групп пользователей
...
if ($_POST['action'] == "show" AND is_numeric($id)) {
    ...
    $messager = $iChat_db->query("SELECT message FROM iChat WHERE id = '$id'");
    
    $message = $messager->fetchArray();
    ...
    $message = preg_replace_callback ("#<!--dle_leech_begin--><a href=[\"'](http://|https://|ftp://|ed2k://|news://|magnet:)?(\S.+?)['\"].*?" . ">(.+?)</a><!--dle_leech_end-->#ie", "\iChat_decode_leech('\\1\\2', '\\3')", $message);
...
if ($_POST['action'] == "save" AND is_numeric($id)) {
    ...
    if (!$error) {
        ...
        if (function_exists("get_magic_quotes_gpc") && get_magic_quotes_gpc())
            ...
            $new_message = htmlspecialchars( $new_message, ENT_QUOTES, $config['charset'] );
        ...
        if ($user_group[$member_id['user_group']]['allow_url']) {
            ...
            $new_message = preg_replace_callback ("#\[leech\](\S.+?)\[/leech\]#i", "\iChat_build_url(array('html' => '\\1', 'show' => '\\1'))", $new_message);
            $new_message = preg_replace_callback ("#\[leech\s*=\s*\&quot\;\s*(\S+?)\s*\&quot\;\s*\](.*?)\[\/leech\]#i", "\iChat_build_url(array('html' => '\\1', 'show' => '\\2'))", $new_message);
            $new_message = preg_replace_callback ("#\[leech\s*=\s*(\S.+?)\s*\](.*?)\[\/leech\]#i", "\iChat_build_url(array('html' => '\\1', 'show' => '\\2'))", $new_message);
        ...
        $iChat_db->query("UPDATE iChat SET message='$new_message' WHERE id = '$id'");
