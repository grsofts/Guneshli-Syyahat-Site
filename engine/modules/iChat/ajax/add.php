...
/*
====================================================
Автор: RooTM http://weboss.net/
----------------------------------------------------
Адаптация под DLE: 10.4 SX2 http://sx2.name/
====================================================
*/
...
if (!$is_logged AND $chat_cfg['allow_guest'] == 'no')
    die("error");
...
if (!$is_logged) {
    ...
    if (strlen($mail) > 50)
        $error = $lang['news_err_2'];
    
    if ($mail != "") {
        if (!preg_match("/^[\.A-z0-9_\-]+[@][A-z0-9_\-]+([.][A-z0-9_\-]+)+[A-z]{1,4}$/", $mail))
            $error = $lang['news_err_10'];
    }
    
    $name = trim($_POST['name']);
    
    if (dle_strlen($name, $config['charset']) > 20)
        $error = $lang['news_err_1'];
    
    if (preg_match("/[\||\'|\<|\>|\[|\]|\"|\!|\?|\$|\@|\/|\\\|\&\~\*\{\+]/", $name))
        $error = $lang['reg_err_4'];
    
    if (empty($name) and $error != TRUE)
        $error = $lang['news_err_9'];
    ...
    if ($error != TRUE) {
        ...
        if ($db->num_rows() > 0)
            $error = $lang['news_err_7'];
        $db->free();
    }
    
    $member_id['foto']       = '';
    $member_id['user_group'] = 5;
    $member_id['name']       = sqlite3::escapestring(addslashes($name));
    $member_id['email']      = sqlite3::escapestring(htmlspecialchars($mail));
...
if (count($smilies_in_msg[0]) > $chat_cfg['max_smilies'])
    $error = $chat_lang['max_smilies'];

if ($is_logged AND in_array($member_id['name'], explode(",", $chat_cfg['no_access'])))
...
if ($StopSpam === true) {
    $error = 'Включена защита от флуда!';
}

if ($message == '' OR $member_id['banned'] == "yes")
    $error = $chat_lang['null'];

if (md5($message) == $_SESSION['last_message_hash'])
...
if ($member_id['user_group'] > 2 and intval($chat_cfg['stop_flood']) and !$error) {
    ...
    $iChat_db->query("DELETE FROM flood where date < '$this_time'");
    $flooder = $iChat_db->query("SELECT COUNT(*) as count FROM flood WHERE ip = '$_IP'");
    
    $row = $flooder->fetchArray();
...
if (!$error) {
    ...
    $_IP   = $iChat_db->escapeString($_SERVER['REMOTE_ADDR']);
    ...
    if ($user_group[$member_id['user_group']]['allow_url']) {
        ...
        function iChat_build_url($url = array())
        {
            ...
            if (iChat_check_home($url['html']) OR $url['html'][0] == "/" OR $url['html'][0] == "#")
                $target = "";
            else
        ...
        $message = preg_replace_callback("#\[leech\](\S.+?)\[/leech\]#i", "\iChat_build_url(array('html' => '\\1', 'show' => '\\1'))", $message);
        $message = preg_replace_callback("#\[leech\s*=\s*\&quot\;\s*(\S+?)\s*\&quot\;\s*\](.*?)\[\/leech\]#i", "\iChat_build_url(array('html' => '\\1', 'show' => '\\2'))", $message);
        $message = preg_replace_callback ("#\[leech\s*=\s*(\S.+?)\s*\](.*?)\[\/leech\]#i", "\iChat_build_url(array('html' => '\\1', 'show' => '\\2'))", $message);
    ...
    $message = $iChat_db->escapeString($message);
    ...
    if ($member_id['user_group'] > 2 and intval($chat_cfg['stop_flood']))
        $iChat_db->query("INSERT INTO flood (date, ip) values ('$_TIME', '$_IP')");
    
    if (!$iChat_db)
        $iChat_db = new SQLite3(ENGINE_DIR . '/modules/iChat/data/iChat.db');
    
    $iChat_db->query("INSERT INTO iChat (date, foto, author, email, message, ip, user_group) values ('$time', '{$member_id['foto']}', '{$member_id['name']}', '{$member_id['email']}', '$message', '$_IP', '{$member_id['user_group']}' )");
    ...
    while ($file = readdir($fdir)) {
        if ($file != '.' and $file != '..' and $file != '.htaccess')
            @unlink(ENGINE_DIR . '/modules/iChat/data/cache/' . $file);
    }
