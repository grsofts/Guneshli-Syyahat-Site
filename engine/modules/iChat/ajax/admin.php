...
if ($_POST['action'] == "save") {
    ...
    if (!is_numeric(trim($save_cfg['guest_refresh'])) OR trim($save_cfg['guest_refresh']) <= 0) {
        $save_cfg['guest_refresh'] = 60;
    }
    ...
    if (!is_numeric(trim($save_cfg['max_smilies'])) OR trim($save_cfg['max_smilies']) < 0) {
        $save_cfg['max_smilies'] = 7;
    }
...
$allow_guest = makeDropDown(array(
    "yes" => $lang['opt_sys_yes'],
    "no" => $lang['opt_sys_no']
), "cfg14", "{$chat_cfg['allow_guest']}");

if (!$iChat_db)
    $iChat_db = new SQLite3(ENGINE_DIR . '/modules/iChat/data/iChat.db');
$counter = $iChat_db->query("SELECT COUNT(*) as count FROM iChat");
$row     = $counter->fetchArray();
...
if ($_POST['check'] == "updates") {
    ...
    if (!$data)
        $data = "Не удалось подключится к удаленному серверу, возможно ваш сервер не поддерживает удаленные соединения, либо произошел сбой в работе наших серверов. Повторите попытку немного позднее ...";
    else {
...
if ($_POST['action'] == "save" OR $_POST['action'] == "clear") {
    ...
    if ($_POST['action'] == "clear") {
        if (!$iChat_db)
            $iChat_db = new SQLite3(ENGINE_DIR . '/modules/iChat/data/iChat.db');
        $iChat_db->query("DELETE FROM iChat WHERE 1");
    }
...
echo "<div id='ECPU' title='{$chat_lang['admin_title']}' style='display:none'>{$content}</div>";
